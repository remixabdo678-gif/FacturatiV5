// src/components/invoices/InvoiceViewer.tsx
...
export default function QuoteViewer({ quote, onClose, onEdit }: QuoteViewerProps) {
  ...
  const createPdfBlob = async (rootEl: HTMLElement, filename: string) => {
    rootEl.classList.add('exporting');
    try {
      const worker = (html2pdf() as any).set(buildPdfOptions(filename)).from(rootEl).toPdf();
      const pdf: any = await worker.get('pdf');
      const blob: Blob = pdf.output('blob');
      return blob;
    } finally {
  ...
  const handlePrint = async () => {
    ...
    try {
      const blob = await createPdfBlob(quoteContent, `Devis_${quote.number}.pdf`);
      openPrintTab(blob, `Devis_${quote.number}`);
    } catch (e) {
      console.error('Erreur impression:', e);
      alert('Erreur lors de la préparation de l’impression');
    }
  ...
  const handleDownloadPDF = () => {
    ...
    const options = buildPdfOptions(`Devis_${quote.number}.pdf`);
    quoteContent.classList.add('exporting');
    (html2pdf() as any)
      .set(options)
      .from(quoteContent)
      .save()
      ...
      .finally(() => quoteContent.classList.remove('exporting'));
  };
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20">
        <div className="inline-block w-full max-w-4xl my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
          ...
          <div id="quote-content" className="p-6 bg-white">
            <TemplateRenderer templateId={selectedTemplate} data={quote} type="quote" includeSignature={includeSignature} />
          </div>
